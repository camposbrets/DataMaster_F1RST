    ...
    create or replace table `projeto-data-master`.`capag`.`report_ind3_cidades_ano`
    ...
    as (
      select
        avg(cast(
            case when replace(a.indicador_3, ',', '.') like '#%' then null else replace(a.indicador_3, ',', '.') end 
